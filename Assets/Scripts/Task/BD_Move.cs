...
public class BD_Move : Action
{
    ...
    private RoleBase m_Enemy;// should be RoleBase
    ...
    public override TaskStatus OnUpdate()
    {
        ...
        FixPosBug();
        if (!IsArrive(m_Target.Value.position, m_Distance.Value))
        {
            MoveByNavPaths(m_Target.Value.position);
            // m_Enemy.Idle();
    ...
    private void FixPosBug()
    {
            ...
            > 1)
        {
            ...
            if (Time.time - tempFixBugTime > 2f)
            {
                ...
                tempFixBugTime = 0f;
                Debug.LogError("not the same pos !!!");
            }
        }
        else
        {
